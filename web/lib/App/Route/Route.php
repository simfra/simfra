...
class Route {
    ...
    private $_modifiers = array("int", "string");
  ...
  //          @include "Asdsads";
    ...
    //    }
      ...
      //  {
            //echo "Adads";
        //}
        if(file_exists(PATH . "App/Struct/struct.php")) {
        ...
        foreach($url_struct as $key=>$link)
        {
            if($link!="") {
                ...
                if($link2['modifier'] =="int" ){
                    if(isset($url[$key]) && ctype_digit($url[$key])) { $temp[] = "[".$link2['name']."|int$parametr]";}
                ...
                elseif($link2['modifier'] == "string"){
                    if(is_string($url[$key])) { $temp[] = "[".$link2['name']."|string$parametr]";}
                    else{ $temp[] = $url[$key];}
                } 
                else{
                    $temp[] = $url[$key];
